...
docker run --rm -v <ttl_directory>:/data pylode:latest <ttl_file> -o <html_file>
```

> Note: `<ttl_directory>` must be absolute; `<ttl_file>` and `<html_file>` are relative to it
...
Here's an example of use with the [AGRIF Ontology](https://linked.data.gov.au/def/agrif) using the source in this repository:
