...
class ValPub:
    ...
    def _make_metadata(self):
        ...
        for s_ in self.ont.subjects(
            ...
            iri = s_
            for p_, objects in ordered_subject_properties(
                self.ont, s_, SHAPES_GRAPH_PROPS
            ):
        ...
        ordered_props = [p for p in SHAPES_GRAPH_PROPS if p in this_onts_props]
        ordered_props.extend(
            sorted((p for p in this_onts_props if p not in SHAPES_GRAPH_PROPS), key=str)
        )
