        ...
        if content_type is None or content_type == "text/html":
            ...
            if css_url or favicon_url or gtag_id:
                ...
                if gtag_id:
                    ...
                    async_tag["async src"] = (
                        f"https://www.googletagmanager.com/gtag/js?id={gtag_id}"
                    )
...
class DocResource:
    def on_get(self, req: falcon.Request, resp: falcon.Response) -> None:
        ...
        sort_subjects = req.get_param_as_bool(
            "sort", required=False, blank_as_true=True, default=True
        )
        profile = req.get_param("profile", required=False, default="ontpub").lower()
        _logger.info(
            f"Processing pyLODE request for '{profile}' rendering of '{url}' with sort_subjects = {sort_subjects}"
        )
        match profile:
            case "ontpub":
            ...
            case _:
                _logger.error(
                    f"Failing pyLODE request for invalid profile parameter '{profile}'"
                )
                raise falcon.HTTPBadRequest(
